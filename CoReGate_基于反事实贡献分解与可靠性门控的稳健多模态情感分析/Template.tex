...
% Clickable hyperlinks (references/citations/URLs)
...
% Keep URL font consistent with the surrounding text
\urlstyle{same}
% Example definitions.
% --------------------
\def\x{{\mathbf x}}
...
On CMU-MOSI, the method achieves the best MAE and Corr, and also performs best under both Has0 and Non0 settings for binary Acc2/F1. Compared with a strong baseline CENET~\cite{wang2022cross}, MAE improves from 0.739 to 0.707 and Corr improves from 0.789 to 0.794, supporting the benefit of gating incremental non-textual evidence on top of a text backbone. While the best Acc7 is achieved by Self-MM~\cite{yu2021learning}, our Acc7 (47.7) remains competitive and is close to the best.

On CMU-MOSEI, the method is strong across most metrics. It ties the best MAE (0.529) and achieves the best Corr (0.777) among compared methods, while remaining competitive on Acc7. It also attains the best Non0 Acc2/F1 (excluding neutral samples), indicating that reliability-gated incremental fusion is particularly beneficial when neutral samples are excluded. The best Has0 binary results are achieved by other methods (e.g., LF-DNN~\cite{williams2018dnn}), suggesting that different evaluation protocols may emphasize different aspects of performance.
...
\begin{table}[t]
  ...
  \begin{tabular}{l|ccc}
    ...
    \hline
    Full (CoReGate) & \textbf{0.7065} & \textbf{0.7936} & \textbf{47.67} \\
    w/o decomposition & 0.7298 & 0.7915 & 45.62 \\
    w/o gating & 0.7211 & 0.7920 & 46.34 \\
  ...
  \begin{tabular}{l|ccc}
    ...
    Setting & MAE$\downarrow$ & Corr$\uparrow$ & Acc7$\uparrow$ \\
    \hline
    Full (CoReGate) & \textbf{0.5288} & \textbf{0.7768} & \textbf{53.10} \\
    w/o decomposition & 0.5347 & 0.7719 & 52.15 \\
    w/o gating & 0.5318 & 0.7728 & 52.48 \\
    w/o branch sup & 0.5362 & 0.7712 & 51.87 \\
    t-only (\(y=y_{\mathrm{T}}\)) & 0.5422 & 0.7665 & 52.31 \\
...
The following observations can be made from Table~\ref{tab:ablation}. First, counterfactual contribution decomposition is effective: compared with w/o decomposition, the full model reduces MAE by \(0.0233/0.0059\), improves Corr by \(0.0021/0.0049\), and improves Acc7 by \(2.05/0.95\) points on MOSI/MOSEI.
Second, reliability gating is beneficial: compared with w/o gating, the full model reduces MAE by \(0.0146/0.0030\), improves Corr by \(0.0016/0.0040\), and improves Acc7 by \(1.33/0.62\) points on MOSI/MOSEI. The benefit becomes even more significant under missing-modality stress tests (Fig.~\ref{fig:robust_missing}), matching the goal of suppressing unreliable increments under missing inputs.
Third, branch auxiliary supervision is important: removing branch supervision (w/o branch sup) increases MAE by \(0.0340/0.0074\), decreases Corr by \(0.0033/0.0056\), and decreases Acc7 by \(2.92/1.23\) points on MOSI/MOSEI. This suggests auxiliary losses help stabilize the learning of \(\Delta_{\mathrm{A}}\) and \(\Delta_{\mathrm{V}}\) for both regression accuracy and correlation.
