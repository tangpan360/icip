...
Counterfactual Contribution Fusion (CCF) for MMSA.

Core idea (counterfactual contribution decomposition):
  - Predict text-only base y_T
  - Predict partial predictions y_TA (remove V), y_TV (remove A)
  - Predict full prediction y_TAV
  - Define counterfactual (marginal) contributions:
      Δ_A  = y_TA  - y_T
      Δ_V  = y_TV  - y_T
      Δ_AV = y_TAV - y_TA - y_TV + y_T    (interaction / synergy term)
  - Learn reliability gates r_A, r_V, r_AV in (0,1) from (features + quality stats)
  - Final prediction:
      y = y_T + r_A * Δ_A + r_V * Δ_V + r_AV * Δ_AV

This keeps the model:
  - alignment-free (works with unaligned A/V via true lengths masking)
  - interpretable (explicit per-modality marginal contributions + gates)
  - simple enough for stable training in MMSA.
"""
...
class CENET_CCF(nn.Module):
    def __init__(self, args):
        ...
        self.head_tav = _MLP(self.text_dim + 2 * self.av_out_dim, head_hidden, self.output_dim, dropout=head_drop)
        # unimodal A/V heads (for ablations)
        self.head_a = _MLP(self.av_out_dim, head_hidden, self.output_dim, dropout=head_drop)
        self.head_v = _MLP(self.av_out_dim, head_hidden, self.output_dim, dropout=head_drop)
        ...
        # - use_gating: if False, set r_A=r_V=1 (and r_AV=1 if interaction enabled), i.e., no reliability gating.
        # - fusion_mode:
        #     "coregate" (default): y = y_T + r_A*Δ_A + r_V*Δ_V (+ r_AV*Δ_AV)
        #     "t_only":            y = y_T
        #     "a_only":            y = y_A
        #     "v_only":            y = y_V
        #     "tav_only":          y = y_TAV
        ...
        if self.fusion_mode not in ("coregate", "t_only", "a_only", "v_only", "tav_only"):
            raise ValueError(f"Unsupported fusion_mode: {self.fusion_mode}")

        self.use_interaction = bool(getattr(args, "use_interaction", True))
        if self.use_interaction:
            # interaction gate uses both modality features and both quality stats
            self.gate_av = _MLP(self.text_dim + 2 * self.av_out_dim + 6, head_hidden, 1, dropout=head_drop)
        else:
            self.register_parameter("gate_av", None)
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        y_tav = self.head_tav(torch.cat([h_t, h_a, h_v], dim=-1))
        y_a = self.head_a(h_a)
        y_v = self.head_v(h_v)

        # Marginal contributions + interaction (inclusion–exclusion)
        d_a = y_ta - y_t
        d_v = y_tv - y_t
        d_av = y_tav - y_ta - y_tv + y_t
        ...
        if self.use_interaction:
            r_av = torch.sigmoid(self.gate_av(torch.cat([h_t, h_a, h_v, q_a, q_v], dim=-1)))  # (B,1)
        else:
            r_av = torch.zeros_like(r_a)
            d_av = torch.zeros_like(d_a)
        ...
        if not self.use_gating:
            ...
            if self.use_interaction:
                r_av = torch.ones_like(r_av)

        # Output selection (ablation)
        if self.fusion_mode == "t_only":
            y = y_t
        elif self.fusion_mode == "a_only":
            y = y_a
        elif self.fusion_mode == "v_only":
            y = y_v
        elif self.fusion_mode == "tav_only":
            y = y_tav
        else:
            y = y_t + r_a * d_a + r_v * d_v + r_av * d_av
