...
class CENET_CCF:
    """
    Trainer for CENET_CCF.

    Primary loss on outs["M"].
    Optional auxiliary branch supervision:
      L = L_task(M, y) + branch_loss_weight * (L_task(y_t, y) + L_task(y_ta, y) + L_task(y_tv, y) + L_task(y_tav, y))

    This keeps counterfactual heads aligned to the task label, improving stability and enabling clean ablations.
    """
    ...
    @staticmethod
    def _zero_like_av(x):
        """Zero-out an AV input while keeping its wrapper (tensor or (tensor,lengths))."""
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False):
        ...
        # Robustness evaluation knobs (applied ONLY for mode == "TEST")
        ...
        # Temporarily disable gating at evaluation if requested (do not affect training/validation).
        ...
        with torch.no_grad():
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    # Apply modality missing (TEST only).
