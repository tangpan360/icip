...
class AMIO(nn.Module):
    def __init__(self, args):
        ...
        self.MODEL_MAP = {
            ...
            'mmim': MMIM,
            'cenet': CENET,
            'cenet_base': CENET_BASE,
            'cenet_tmcc': CENET_TMCC,
            'cenet_ccf': CENET_CCF,
            'almt': ALMT,
            # multi-task
            'mtfn': MTFN,
        ...
        if args.model_name in ('cenet', 'cenet_base'):
            ...
            if args.model_name == 'cenet':
                ...
                # skeleton: no extra linguistic embeddings, no CE
                self.Model = CENET_BASE.from_pretrained(args.pretrained, config=config, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False, args=args)
        else:
